...
if num_date_args == 0:
    ...
    if 'n' in args[0]:
        num_days = int(args[0].replace('n',''))
        start_date = None
    else:
        start_date = end_date = \
            datetime.date(TODAY.year, TODAY.month, int(args[0]))

elif num_date_args == 2:
    if 'n' in args[1]:
        start_date = datetime.date(TODAY.year, TODAY.month, int(args[0]))
        num_days = int(args[1].replace('n',''))
        end_date = None
    else:
        start_date = end_date = \
            datetime.date(TODAY.year, int(args[0]), int(args[1]))

elif num_date_args == 3:
    arg_1 = int(args[0])
    if arg_1 > 99:
        start_date = end_date = \
            datetime.date(arg_1, int(args[1]), int(args[2]))
    else:
        year = TODAY.year
        start_date = datetime.date(year, arg_1, int(args[1]))
        if 'n' in args[2]:
            num_days = int(args[2].replace('n',''))
            end_date = None
        else: end_date = datetime.date(year, arg_1, int(args[2]))
    ...
    if num_date_args == 4:
        if arg_1 > 99:
            month = int(args[1])
            start_date = datetime.date(arg_1, month, int(args[2]))
            end_date = datetime.date(arg_1, month, int(args[3]))
        else:
            year = TODAY.year
            start_date = datetime.date(year, arg_1, int(args[1]))
        ...
        start_date = datetime.date(arg_1, int(args[1]), int(args[2]))
        end_date = datetime.date(arg_1, int(args[3]), int(args[4]))
...
if success < 0:
    ...
    print '    %s thru %s' % (timeString(analysis_start),
                              timeString(end_time))
...
if debug:
    ...
    print '      num days :', analysis_end - analysis_start
...
# run the model's stress hours index calculator
stress_hours = model.stressHours(*model_data)
max_stress = N.nanmax(stress_hours)

if debug:
    print '\nstress_hours grid shape :', stress_hours.shape
    if max_stress > 0:
        print '\nStress hours summary :'
        for hours in range(max_stress, -1, -1):
            count = len(N.where(stress_hours == hours)[0])
            print '    %6d occurrences of %d stress hours' % (count, hours)

# update the stress hours index dataset 
manager = factory.threatFileManager(threat_key, period_key, start_date.year)
index_end_date = manager.updateReanalysis('stress', start_date, stress_hours)
manager.close()
...
if max_stress > 1:
    ...
    print '\nNo heat stress hours occurred in this time span.'
    risk_level = N.zeros(stress_hours.shape, dtype='<i2')
...
if risk_end_date != index_end_date:
    print '\nERROR : THREAT DATA END DATES DO NOT MATCH :'
