...
def modelDataStartTime(factory, threat_key, date):
...
def reportUpdate(source, threat_fullname, period_key, start_date, index_end, risk_end):
    info = (threat_fullname, period_key.title(), source)
    print '\nCompleted update of %s %s %s risk.' % info
...
def updateReanalysis(factory, threat_key, period_key, start_date, end_date, verbose, debug):
    start_time = modelDataStartTime(factory, threat_key, start_date)
    ...
    if debug:
        ...
        print 'Processed %d raw data hours: %s thru %s' % info
...
common_fcast_end = factory.commonFcastEnd(threat_key, TODAY)
if common_fcast_end.day == LAST_DAY_OF_MONTH.day and common_fcast_end.hour == 23:
    next_common_end = factory.commonFcastEnd(threat_key, FIRST_DAY_NEXT_MONTH)
    if next_common_end is not None: common_fcast_end = next_common_end
max_fcast_time = maxForecastEndTime(factory, threat_key)

if debug:
    print '\n   commonFcastEnd :', common_fcast_end
...
for period_key in periods:
    ...
    if replace_prev:
        ...
        print 'replacing previous reanalysis for :', start_date
        risk_start, risk_end = updateReanalysis(factory, threat_key, period_key, start_date, end_date, verbose, debug)
        reportUpdate('forecast', threat_fullname, period_key, risk_start, index_end, risk_end)
        ...
        if last_obs_date < max_obs_date:
            ...
            if verbose: print 'updating reanalysis for :', start_date, end_date
            risk_start, risk_end = updateReanalysis(factory, threat_key, period_key, start_date, end_date, verbose, debug)
        else:
            print '\n%s reanalsysis data is already up to date' % threat_fullname
            risk_end = last_obs_date

    # run the model with a forecast
    fcast_start = risk_end + ONE_DAY
    fcast_start_time = modelDataStartTime(factory, threat_key, fcast_start)
    ...
    if verbose:
        print '\n Updating %s %s risk forecast :' % (threat_fullname, period_key.title())
    ...
    reportUpdate('forecast', threat_fullname, period_key, fcast_start, index_end, risk_end)
    if debug:
