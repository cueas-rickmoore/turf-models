...
STRING_TO_TYPE = { 'N.int8':N.int8, 'N.int16':N.int16, 'N.int32':N.int32,
                   'N.int64':N.int64, 'N.uint8':N.uint8, 'N.uint16':N.uint16,
                   'N.uint32':N.uint32, 'N.uint64':N.uint64,
                   'N.float16':N.float16, 'N.float32':N.float32,
                   'N.float64':N.float64, 'N.bool':N.bool,
                   'N.nan':N.nan, 'None':None }

TYPE_TO_STRING = { N.int8:'N.int8', N.int16:'N.int16', N.int32:'N.int32',
                   N.int64:'N.int64', N.uint8:'N.uint8', N.uint16:'N.uint16',
                   N.uint32:'N.uint32', N.uint64:'N.uint64',
                   N.float16:'N.float16', N.float32:'N.float32',
                   N.float64:'N.float64', N.bool:'N.bool',
...
class GridFileBuildMethods:
    ...
    def buildDataset(self, dataset_key, group_path=None, group_keydict=None,
        ...
        if chunks is None: chunks = self._estimateChunks(attrs['view'], shape)
    ...
    def _resolveDatasetBuildAttributes(self, dataset_config, keydict=None,
        ...
        packing_attrs = self._resolveDatasetPackingAttributes(dataset_config)
        dtype_packed = packing_attrs['dtype_packed']
        del packing_attrs['dtype_packed']
        attrs.update(packing_attrs)

        return shape, dtype_packed, attrs

    # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

    def _resolveDatasetPackingAttributes(self, dataset_config):
        attrs = { }

        missing = dataset_config.get('missing_data', None)
        missing_packed = dataset_config.get('missing_packed', None)
        if missing_packed: attrs['missing'] = missing_packed
        ...
        units_packed = dataset_config.get('units_packed', None)
        units = dataset_config.get('units', None)
        if units_packed: attrs['units'] = units_packed
        else:
            if units:
                if multiplier:
        ...
        dtype_packed = dataset_config.get('dtype_packed', dtype)

        if dtype != dtype_packed:
            if units_packed and units_packed != units:
                params = (dtype, missing, units)
            else: params = (dtype, missing)
            unpack = [ ]
            for value in params:
                _str_ = TYPE_TO_STRING.get(value, 'value_not_found')
                if _str_ == 'value_not_found': unpack.append(str(value))
                else: unpack.append(_str_)
            attrs['unpack'] = '(%s)' % ','.join(unpack)

        attrs['dtype_packed'] = dtype_packed
        return attrs
